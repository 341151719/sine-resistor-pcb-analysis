...
证据纠正：2026-09-15，见 [重新审查报告](../主动审计/EVIDENCE_REASSESSMENT_20260915.md)。旧日志保留历史数值，新 deck 增加平均电流测量。
...
| OPA ON 后约 70 mA CC | **CC 尚未复现，只复现高频负载电流** | 2 Vpk/105k：正轨峰值约 69 mA，平均约 23.6 mA；缺少台式电源动态 |
...
适用性限制：OPA548 推荐供电最低为 ±4 V；上述 ±1～±3 V 点属于范围外探索，不能用其阴性结果排除实板低压异常。

## 4. OPA ON 后高频取电：峰值和持续 CC 必须分开

文件：`02_opa_on_105k_amp{1,2,3,4}V.cir`

这里把实板后来观测到的高频 VCMD 当作已经存在的条件，检查 OPA548 打开后的瞬时与平均取电。模型不包含电源限流控制，不能直接判定 CC。
...
2026-09-15 复跑 2 Vpk 案例：正/负轨平均取电约 23.62/23.48 mA，远低于其 69.14/68.75 mA 峰值。原先将这一峰值当成持续 70 mA CC 的解释，证据不足。

真实供电还包含 C5/C6=470 µF、线缆和台式电源控制环路。高频瞬时电流与电源显示/限流响应不能等同。新 deck 输出 `IP_DRAW_AVG`、`IN_DRAW_AVG`；这些平均值也只适用于当前理想电源模型，不能反过来证明实板绝不会 CC。
