...
- 原修正deck用PWL让四路理想电源同时从0 V上升；10 Ω点在未供电宏模型的初始求解中止。
- 单故障假设声明的是固定J2=±10 V、J1=±5 V。改用固定偏置后，RAPS=3.5586 mΩ/1 Ω/10 Ω均完成约0.5 ms瞬态。
- 三例TP8晚段均0 Vpp；U4+终值约9.998/9.990/9.920 V。

因此 H07 现在支持“在固定偏置和当前宏模型中，该聚合串联高阻没有产生目标振荡”。
它不覆盖上电顺序和实际电源限流，也不能把聚合RAPS定位到具体FB1、铜路或焊点。
